...
program sbbq2, byable(recall) sortpreserve
	version 9.2
	...
	tempvar t
	g `t' = _n
	cap g `varlist'_point = .

	mata: `varlist' = st_data( ., "`varlist'", "`touse'")
	cap mata: point=mbbq(`varlist',`phase',`window',`cycle')
	cap mata: st_store(., "`varlist'_point", st_varindex("`touse'"), point)
